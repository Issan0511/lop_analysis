...
OUT=ROOT/'results/unit_fates_0913/normal_snake_ablation_0914'
def main():
 OUT.mkdir(exist_ok=True)
 paths=[DATA/'ckpts'/f'SN_normal_q0_step{s}.pt' for s in [0,200000,1000000,5000000]]
 ...
 for step in [200000,1000000,5000000]:
  ...
  for ri in range(10):
   with np.load(DATA/'logs'/f'SN_normal_q0_seed{ri}.npz') as log:
 ...
 for task in [20,100,500]:
  for label in masks:
   ...
   report.append(dict(task=task,intervention=label,**agg));print(report[-1])
