"""snake_lr_diag_0903 — Snake を低 lr で完走させる診断走（**未登録・事後**）。

    OMP_NUM_THREADS=1 python3 -m src.snake_lr_diag_0903 --stage run --lr 0.005
    OMP_NUM_THREADS=1 python3 -m src.snake_lr_diag_0903 --stage analyze --lr 0.005
...
ARM = "SN_a1_1216"
REGISTERED_LR = 0.01
UNREGISTERED = ("この走は事前登録されていない。Issa の 2026-09-04 の指示で回した診断で、"
                "変えた軸は lr 1 本だけ。verdict には入れず、他腕との水準比較にも使わない。")


def _outdir(lr: float) -> Path:
    tag = ("%g" % lr).replace(".", "p")
    return Path(ROOT) / "results" / "_diag_snake_lr_0903" / f"lr{tag}"


def _cfg_for(lr: float) -> dict:
    cfg = load_config(str(STAGE_A_CONFIG))
    if float(lr) == REGISTERED_LR:
        raise ValueError("registered lr は登録走が持っている。診断は lr を変えたときだけ")
...
def run(lr: float) -> dict:
    ...
    prov = dict(
        ...
        arm=ARM, activation="snake", act_alpha=1.0,
        lr=float(lr), registered_lr=REGISTERED_LR,
        changed_axes=["common.lr_main"],
    ...
    print(f"[{EXPERIMENT}] lr={lr:g} -> {got.get('status')} "
          f"in {prov['elapsed_sec']:.1f}s", flush=True)
    return prov


def analyze(lr: float) -> dict:
    cfg = _cfg_for(lr)
    outdir = _outdir(lr)
    ...
    result = dict(
        experiment=EXPERIMENT, unregistered=True, lr=float(lr), arm=ARM,
    ...
    (outdir / "summary.md").write_text(
        f"# {EXPERIMENT} — Snake を lr={lr:g} で完走させる診断\n\n"
        f"> **{UNREGISTERED}**\n\n"
        f"- 登録走の `SN_a1_1216`（lr {REGISTERED_LR:g}）は "
        f"`NUMERIC_DIVERGENCE`（step 1,000・seed 3）\n"
        f"- 本診断（lr {lr:g}）: S-cap `{cap['status']}` / "
