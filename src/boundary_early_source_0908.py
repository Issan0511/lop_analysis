"""Early source replay: training loop copied from boundary_groups_0908; only logging/window changed."""
...
OUT=ROOT/'results/boundary_early_0908/source'
...
def run(arm,seed,iv,mnist,tasks=20,first=1,audit=True,persist=True):
 ...
 per=[];position=[];raw=[];first_task={};oldperm=None;start=time.monotonic()
 for task in range(1,tasks+1):
  ...
  if task==1 and audit:
   first_task={'perm':perm.clone(),'before':obs(params,act,px[:,perm],mu)}
  if dense:
   with torch.no_grad():
  ...
  for step in range(625):
   ...
   if task==1 and audit and step+1 in [20,625]:
    first_task['after_'+str(step+1)]=obs(params,act,px[:,perm],mu)
   if dense and step<300:
    with torch.no_grad():z1,z2=zbar(params,px[:,perm],act)
    d1.append(z1);d2.append(z2)
   if dense and audit and step+1 in [20,300,625]:
  ...
  oldperm=perm
  if persist and task%5==0:print(arm,iv,seed,'task',task,'elapsed',round(time.monotonic()-start,1),flush=True)
 ...
 if persist:
  ...
  torch.save({'boundaries':raw,'probe_indices':pidx,'mu_mean':mu,'first_task':first_task},OUT/(prefix+'_states.pt'))
  cmp={}
  meta={'arm':arm,'iv':iv,'seed':seed,'device':'cpu','threads':1,'source_code_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest(),
        ...
        'original_exact':None,'window':'tasks2..20; no original CUDA comparison','data_sha256':mnist.sha256,'wall_seconds':time.monotonic()-start,
        'spec_sha256':hashlib.sha256((ROOT/'specs/spec_boundary_early_0908.md').read_bytes()).hexdigest()}
