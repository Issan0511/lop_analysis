...
| # | 予測 | 確率 |
|---|---|---|
| P1 | `R` はこの箱では崩壊しない（窓の 10 seed 中央値 ≥ .5） | **95%** |
| P3 | `RH − R` が登録の「勝ち」になる | **未登録**（本走開始前に決める） |

P1 の根拠（本人の議論を記録する）: `relu_doors_0919` の 3072-100-100-10 では
`act → fc → act → fc` が**正規化なしで直列**につながっており、per-unit の DC ずれが層をまたいで
積み上がる経路があった。ViT では各 FFN の中に活性化が 1 つしかなく、block をまたいで
次の活性化へ行く経路には**必ず `norm2` の LayerNorm が 1 枚挟まる**。
DC が直列に積み上がる箇所が構造的に存在しないので沈下は起きない、という読み。

この議論の過程で Issa は一度「この箱でも崩壊する」と述べ、FFN が 2 層 MLP
（384→1536→384）であることを確認した後に「崩壊しない」へ変えた。**最初の向きも記録しておく。**

Claude（本セッション）の予測: 上表の依頼元の値に対し P1 を 88%、P3 を 18%、P4 を 15% に置いた。
その後 Issa の指摘（LN の位置ではなく **act→act 経路に LN が挟まること**が要点）を受けて
P1 を上げるべきだと判断したが、**値を見る前の記録としては 88% のまま残す**。
