...
状態: **事前登録。設計は Issa が原案を採用し、予測候補と GO も承認済み（§6.3）。実装・検査走・本走は未着手で、当初依頼どおり別途の実装指示待ち。**
...
手続きの型: `specs/spec_relu_doors_0919.md` と `specs/spec_resp_ee_0917.md`。§6 の予測と GO は記録済み。今回の担当範囲は spec の commit までで、実装・本走は別途の指示の後に進める。
...
### 6.3 Issa（**設計・予測候補の採用、GO を記録済み**）

- 記名・時刻（JST）: **Issa**。記録時刻 **2026-09-19T23:42:02+09:00**、実装前・延長結果の生成前。下記の明示承認を Codex が転記。
- CH の型 / CHB の型: **CH = GENTLE、CHB = HOLDS または GENTLE**。CH の bias は動くが t200 では崩壊まで届かず、CHB の低下は CH 以下と予測する。
- 主ラベル / OMEGA_ONLY の有無: **CURED_200 ＋ OMEGA_ONLY**。応答の個数条件は原案のまま（GENTLE の dead・gate は t50 から増えない）。
- CH の t181–200 の低下予測: **t31–50 から 1〜2ポイント以内の低下**。目安を 1〜2 pt、上限を 2 pt とする。数値予測の採点は §4.1 の seed 内窓差を seed 平均した `0≤mean_s D_CH,s≤0.02`、二腕の順は `mean_s D_CHB,s≤mean_s D_CH,s`。型・主ラベルは §5 の登録規則で別々に採点する。
- 設計分岐の判断（下表）: **全部原案で**（Issa の明示指示。記録時刻 2026-09-19T23:37:08+09:00、Codex が転記）。
- GO（対象 spec commit）: **GO**。設計は `e9dda9eb1fecf5e4327927834305e54f0aa813e9`（初稿 `77e70f63aac65427fababfb288d53ef463f50611` の判定条件を維持）に対するもの。予測は上記候補を採用し、この追記 commit で登録する。

**予測・GO の出所**: Codex がチャットで上記の予測候補を示し、Issa が「この候補でGOでいいんですが」と明示承認した。独立に作られた Issa の予言ではなく、提示された候補の採用である。候補説明で併記した 0.55 / 0.30 / 0.15 は §6.1 の起案 Claude の確率として紹介したもので、Issa 独自の確率は付けていない。**今回の仕事は spec の commit まで。** 実装・検査・本走は当初依頼どおり別途の実装指示の後に進める。延長結果を一行でも見た後は本稿の閾値・ラベルを変更しない。

| 起案時に提示した分岐（原案採用） | 本稿の具体案 |
|---|---|
...
**採用記録（2026-09-19、実装・延長結果の生成前）**: Issa が上の判断点の説明と深さの線の説明を受けて「全部原案で」と指定した。対象は初稿 commit `77e70f63aac65427fababfb288d53ef463f50611` の設計。HOLDS の片側帯、dead・gate の増加を個数でゼロとする条件、ω の最大～和の帯、深さだけでも COLLAPSE とする条件と DEPTH_ONLY の併記、9/10 seed 対の支持と保留分岐をそのまま採用する。説明時に Codex が提案した「揺らぎ分を許す」変更は採らない。GPU 逐次・他走と競合させない計画と、独立監査の有無を明記する原案の運用も維持する。独立監査の実施・担当割当はこの指示から推測しない。**設計の採用を Issa の結果予測や実装・本走の GO と読み替えない。** 判定式・閾値・既存の予測は変更していない。
...
- 今回は **spec だけ**を作成した。新しい学習、checkpoint ロード、GPU 検査、t51 以後の観測はゼロ。Codex の予測は資料閲覧前に固定したが、依頼文の既知結果に条件付いている。起案 Claude の時刻は不明。初稿では Issa は未記入だったが、その後の候補採用と GO を §6.3 に時刻・出所つきで追記した。
