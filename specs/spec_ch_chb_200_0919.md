...
状態: **事前登録案を commit。設計は Issa が原案を採用（§6.3）。Issa の予測・GO 待ち。実装・検査走・本走は未着手。**
...
**予測・GO 欄を Codex が代筆・推測して埋めない。今回の仕事は spec の commit まで。** Issa の予測、GO、別途の実装指示の後に実装・検査・本走へ進む。GO 前の修正は追加 commit とし旧案も残す。延長結果を一行でも見た後は本稿の閾値・ラベルを変更しない。
