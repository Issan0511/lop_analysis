...
状態: **事前登録 spec。Issa の予測・GO を記録済み（§6.2）。この commit 時点で実装前・未実行。**
...
### 6.2 Issa（実装前に予測と GO を記録）

記名: Issa（チャットから Codex が転記）

記録時刻（JST）: 2026-09-19 23:46:24 JST（転記時刻。発言時刻を推定していない）

実装前・H の結果未読の確認: 本セッションで H の実装・検査・本走をまだ開始していない時点で受領。

| 項目 | Issa の予測（確率は付けない） |
|---|---|
| M_H | **`H_ALONE_FAILS`**（①救えない） |
| N_C | **`C_NEEDED`**（②必要） |
| R_LAYER | **`LAYER_CORRESPONDENCE`**（③成立する） |

GO: Issa「①救えない、②必要、③成立する」「GOして実行」。上記時刻に転記。

元の起案時は本欄を空けて停止した。その後 §6.3 の3点を採用し、今回予測と明示のGOを受領したため、この追加commitを最終登録commitとして実装・検査・本走へ進む。§2・§8・§10の「今回は実装しない／specまで」は起案時の記録として残す。判定・閾値・腕・seed・窓は変更しない。

### 6.3 設計上の分岐点（Issa：3点とも本案を採用）

**裁定記録（Codex転記：2026-09-19 23:43 JST、実装前・H未実行）**: Issa「3点とも本案どおりで」。下表の「本案に登録した選択」を3点とも採用する。§5の帯・判定規則、親と同じHの範囲、再利用検査不成立時の停止方針を確定した。これは設計の採用であり、§6.2のIssaの予測や実装・本走のGOを代記するものではない。起案時の分岐表は以下に残す。
