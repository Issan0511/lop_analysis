...
## 追補9 — 新マシンでは全条件を最初から実行（追補8の途中再開方針を変更）

ユーザーとの相談により、新マシンで全13活性化×seed0–9をSNA seed0 task1から取り直す。ハードウェア・ソフトウェア環境を全条件で揃えるため、旧試行18完了run・751 taskの途中データは保管し、新本走の集計に混ぜない。旧checkpointからtask32へ継続する案は採用しない。
コード・高速化・学習条件・統計判定は再利用し、新環境で検査後、空のraw/runsで開始する。旧raw約49GiBの転送は新本走には不要。元マシンはSTOPのまま。新本走はまだ開始していない。
