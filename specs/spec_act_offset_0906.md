...
親: [[命題1-5_上端則_spec_0905]]（結果 §10.3: 活性化方向へ引き戻す復元場は実在・固定点は場の零点）／[[前活性の力学_事後_0904]] §8（ビー玉 R = E[φ²] + κE[φ′²]） / 状態: **実行済み（2026-09-06 19:14 完走・lab・登録判定 `results/act_offset_0906/verdict.csv`・結果ノート vault `測定/前活性オフセット結果_0906.md`。ラダー A `NOT_DETERMINED`（c=±2 が lr 0.01 で発散）／ラダー B（追補 1）`OFFSET_SIGNED_B`）** / 出典チャット: `活性化プロット_0904`（09-06 夕・Issa の 2 候補）
