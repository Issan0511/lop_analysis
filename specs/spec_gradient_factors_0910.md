...
## 追補 1（2026-09-11・**走の前**・判定値は未読）

§5 の C1/C2 に書いた許容 ≤1e−12 は誤り。**宿主 `pmnist_boundary_host_0908` は float32 で走る**ので、autograd が返す ∂L/∂W1 自体が相対 1e−7 程度の誤差を持ち、16 標本の和での桁落ちを含めるとスモークで 1.9e−7〜2.1e−5 だった。float64 で再構成した積と比べる以上、これが下限になる。

**許容を 1e−3 に改める。** 変異対照（ゲートを外す）はスモークで 60〜3180 なので、分離は 4 桁以上残る。

あわせて §5 の C1 と C2 を 1 本にまとめる。4 因子は $\xi$ と $R$ を $p,e,X$ から**独立に**計算するので、積 $g2\cdot e2\cdot\xi\cdot R$ を autograd の $\lVert\nabla W_1\rVert^2$ と比べる検査は恒真ではない（どれか 1 つが誤っていれば落ちる）。$R$ が $K$ のスペクトル内にあること（Rayleigh 商の束縛）を独立な検査として追加する。
