...
## 追補 2（2026-09-11・**本走の前**・判定タスク t=100,120 の値は未読）

初回の本走を 36 走中 16 走で中断し、破棄して再走する。理由:

恒等式の相対誤差をユニットごとの $\lVert\nabla W_{1,i}\rVert^2$ で割っていたため、**勾配が float32 で厳密に 0 になる死んだユニット**（`ELU1` など、$\varphi'=e^z$ が下位桁で消える）で分母が下限 1e−300 に張り付き、`ident` が 1e+284 に発散して assert が落ちた。

**分母をそのステップの層内最大 $\max_i\lVert\nabla W_{1,i}\rVert^2$ に改める**（層の尺度に対する相対誤差）。スモークで `ident` は全腕 1e−7 台、変異対照（ゲートを外す）は 10.4〜14.7 で 8 桁の分離が残る。中断した 16 走の結果ファイルは削除し、全 36 走を新しいコードで回す。

**開示**: スモーク（t=21 まで・TRACK={20}）で `LR` `BL050` `BL010` `SN06` `ELU1` `LIN` `BL100` の t=20 の `graw`・`rho`・`phi2_probe` を見ている。判定タスク（t=100,120）の値ではないが、`BL050` の t=20 の ρ が `LR` より**高かった**（39.4 対 35.2）ことは V3 (ii) の予測に不利な兆候として既に見えている。**判定・帯・予測はいずれも変更していない。**

## 追補 3（2026-09-11・**V1 の再計算のための再走**・V1 の判定値は未読）

初回の 36 走は完走し検査も全て通ったが、**V1 が事前登録した量を `ELU1` で計算できていなかった**。

$\log\lVert\nabla W_1\rVert^2$ の腕間分散を 4 因子へ加法分解するには、ユニット平均を log 空間で取る必要がある。実装は下限 1e−300 でクランプしてから log を取っていたため、**φ′ = e^z が float32 で 0 に落ちる死んだユニット**（`ELU1` の深く沈んだ側）で $\log g2$ と $\log\lVert\nabla W_1\rVert^2$ が別々の場所にクランプされ、加法性が壊れた。実測の破れは `ELU1` で **−39.5**、他の 11 腕は 0（±1e−15）。寄与率の和が 1 にならず 0.132 だった。

**修正**: 4 因子と $\lVert\nabla W_1\rVert^2$ がすべて厳密に正のユニットだけで log 平均を取り、残したユニットの割合 `alive` を記録する。加法性の破れ `addgap` を検査に追加（許容 1e−6 = exp/log の往復誤差）。

**再走の範囲**: 全 36 走。軌道は一切変わらない（G1・B0・C4 が同じ値で通ることで確認する）。**V2–V5 の判定値は既に見ているが、いずれも本修正と無関係な量で、帯・予測・ラベル定義はどれも変更していない。V1 の値だけが未読である。**

初回走の結果ファイルは破棄する。

## 追補 4（2026-09-11・V1 の判定値は依然未読）

追補 3 の修正でも `ELU1` で加法性が 0.042 残り、22 走目で assert に落ちた。原因は死んだユニットではなく**基準の取り方**だった。

4 因子は float32 の $p,e,X$ から **float64 で** 計算するので、その積は autograd が返す **float32 の** $\lVert\nabla W_{1,i}\rVert^2$ より正確である。層内最大より何桁も小さい勾配を持つユニットでは autograd 側が桁落ちし、$\log$ がその誤差を拡大していた。

**分解の基準を float64 の積に置く。** autograd との突き合わせは `ident`（層内最大に対する相対、実測 1e−7 台、変異対照 6.6–14.7）が担う。対数を取れないユニット（`ELU1` の $\varphi'=e^z$ が float32 で 0 に落ちる側）だけを除き、残った割合を `alive` に記録する。スモークで `addgap` は 0〜4.4e−15。

全 36 走をやり直す（通算 3 度目）。軌道は一切変わらない。**V1 の値は未読のまま。V2–V5 は既に見ているが本修正と無関係で、帯・予測・ラベル定義は一切変更していない。**
