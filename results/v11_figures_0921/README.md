# V11 原稿の図 — 正式番号 1–10（0923・通し稿 v1）
...
体裁は vault `可塑性喪失/論文作成/V11原稿_図表一覧と9月30日までの計画_0921` §2.5 の作図規約に従う。
...
凍結窓の介入を図 6 とし、旧図 6–9 を図 7–10 に移した。ファイル名は既存の識別子として
維持し、画像内の見出し・キャプション・付録 A の参照を更新した。以下の 0922 の記録は当時の番号。
**図 6 は未作図**で、現在の `make_all.py` は作成済みの図だけを再生成する。

| 正式番号 | 内容 | ファイル ID（PNG・PDF・TXT） |
|---|---|---|
| 1 | 幾何と初期配置 | `fig01_geometry` |
| 2 | 中心化の扉 | `fig02_doors` |
| 3 | bias 経路 | `fig03_bias_route` |
| 4 | 実ラベルでの扉 | `fig04_real_doors` |
| 5 | 駆動 | `fig05_drive` |
| 6 | 凍結窓の位置 | 未作図。予定 ID `fig5s_switch_push`、仕様は Vault 図表一覧 §2.6 |
| 7 | W 増大 → 進行 | `fig06_growth` |
| 8 | 活性化 | `fig07_activation` |
| 9 | 応答 → 再学習能力 | `fig08_response` |
| 10 | 実ラベルの箱 | `fig09_5p1` |

決定 11 は改訂決着: S-A の 7 セルは付録 J、両層の傾きの床は本文 §6 の文、
追加の凍結窓介入は §4 の独立した図 6。活性化の図 8 にパネルは足さない。
図 10 のキャプションでは λ の説明も本文に合わせて訂正した
（原典の値は 1e−2、箱ごとに調整した値は 1e−3）。測定値・集約・描画データは変更していない。
