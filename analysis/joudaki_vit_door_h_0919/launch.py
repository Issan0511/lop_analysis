...
def main():
    ...
    for filename in ('checks_compile.json', 'speed_checks.json', 'report_checks.json'):
        check = REPO / 'results/joudaki_vit_door_h_0919' / filename
    ...
    for seed in range(10):
        for arm in ARM_ORDER:
            ...
            if (out / 'done.json').exists():
                continue
            command = [sys.executable, '-m', 'analysis.joudaki_vit_door_h_0919.run',
            ...
            for attempt in range(3):
                ...
                time.sleep(30)
    subprocess.run([sys.executable, '-m', 'analysis.joudaki_vit_door_h_0919.report'], cwd=REPO, check=True)
    atomic_json(RAW / 'status.json', dict(status='complete', updated=time.time(), report=str(REPO / 'results/joudaki_vit_door_h_0919/summary.md')))
