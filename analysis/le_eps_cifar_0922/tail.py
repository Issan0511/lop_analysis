#!/usr/bin/env python3
"""Per-arm, per-seed, per-task snapshot quantities for le_eps_cifar_0922 (spec §3).

    python3 analysis/le_eps_cifar_0922/tail.py            # -> results/le_eps_cifar_0922/tail.csv

Arms LE_e8 / LL_e8 are S-A's runs (results/layer_chimera_cifar_0921, snapshots in the
obsidian-research-data archive); the others are this experiment's.  Everything is recomputed
...
ARMS = {"LE_e8": ("LE", 1e-8, SA_RAW / "LE"), "LE_e4": ("LE", 1e-4, OUT / "LE_e4"),
        "LE_e3": ("LE", 1e-3, OUT / "LE_e3"), "LL_e8": ("LL", 1e-8, SA_RAW / "LL"),
        "LL_e4": ("LL", 1e-4, OUT / "LL_e4"), "LL_e3": ("LL", 1e-3, OUT / "LL_e3")}
EPS_OPEN = float(np.log(1e-8))          # e^{z2} > Adam's parent eps
COLS = ["arm", "cell", "eps", "seed", "task", "open_frac", "n_img", "n_unit", "npos", "apos",
...
def one_arm(arm, cell, eps, root, cifar, tasks, dev):
    ...
    for seed in range(10):
        ...
        for t in range(tasks + 1):
            ...
            r = {"arm": arm, "cell": cell, "eps": eps, "seed": seed, "task": t,
...
def main():
    ...
    for arm in a.arms.split(","):
        cell, eps, root = ARMS[arm]
        rows += one_arm(arm, cell, eps, root, cifar, a.tasks, dev)
    ...
    (OUT / "tail_provenance.json").write_text(json.dumps(
        {"run_id": "le_eps_cifar_0922", "stage": "tail", **C.git_state(), "arms": a.arms,
