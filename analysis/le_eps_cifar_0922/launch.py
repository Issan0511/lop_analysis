#!/usr/bin/env python3
"""Job scheduler for le_eps_cifar_0922 (spec §2), forked from layer_chimera_cifar_0921/launch.py.  Start it detached:

    setsid nohup python3 analysis/le_eps_cifar_0922/launch.py --plan PLAN.json > LOG 2>&1 &

PLAN.json: {"gpu_max": int, "own_max": int, "reserve_gb": float, "min_gpu_mb": int, "retries": int,
            "jobs": [{"arm", "cell", "eps", "rss_gb", "threads"}, ...]}
One job = one arm = one process of src/layer_chimera_cifar_0921.py run --cell CELL --eps EPS --out OUT/ARM
    ...
    while True:
        ...
        if not (L / "STOP").exists() and pending and len(running) < plan["own_max"]:
            ...
            if gpu_n < plan["gpu_max"] and avail - j["rss_gb"] >= plan["reserve_gb"]:
                name = j["arm"]
                cmd = [sys.executable, str(RUNNER), "run", "--cell", j["cell"], "--eps", str(j["eps"]),
