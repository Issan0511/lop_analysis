...
def write_summary(sanity, V, s3, curve, meta):
    ...
    L += ["", "## 3. 事前予測との照合", "",
          ...
          "",
          f"ℓ=2 の ρ(τ) は τ=1..20 平均 {np.nanmean(curve['lag2'][0][:20]):+.4f} → τ=81..100 平均 "
          f"{np.nanmean(curve['lag2'][0][80:]):+.4f} と、大きさが 1/3 ほど縮むだけで**τ=100 でも明確に負のまま**。"
          "spec §3 は「τ≈100 では台帳はほぼ回復済み」を前提に主窓を構造側の読み出しとしたが、"
          "**台帳の半減期 35 step が正しければ τ=96..100 では台帳寄与はほぼ消えているはずで、実測の縮小幅は "
          "それより小さい**。時定数による窓の分離が意図どおり効いているとは、この曲線からは言えない。したがって主窓の値を「構造側だけの保持」と "
          "読むことは本走の証拠では支持されない（§4-2 の「窓の選択は間接的な分離であって代数的な分離ではない」が "
          "そのまま効いている）。一方 ℓ≥3 は τ=1..20 平均 "
          f"{np.nanmean(curve['lag3plus'][0][:20]):+.4f} → τ=81..100 平均 {np.nanmean(curve['lag3plus'][0][80:]):+.4f} "
          "と形が違い、小 τ でゼロ付近・大 τ で弱く負。**R1 が読んでいるものの内訳は本走では確定できない。**",
          "",
