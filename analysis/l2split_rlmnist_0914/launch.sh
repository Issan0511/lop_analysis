...
# waits before it starts until MemAvailable >= 4 GiB + 1.2 x peak RSS (other sessions share this machine).
...
slots_now() { awk -v peak="$PEAK" -v cap="$CAP" '/^MemAvailable:/{n=int(($2/1048576-4.0)/(1.2*peak)); if(n>cap)n=cap; if(n>16)n=16; if(n<0)n=0; print n}' /proc/meminfo; }
N=$(slots_now)
...
run_job() {
  ...
  while awk -v peak="$PEAK" '/^MemAvailable:/{exit !($2/1048576 < 4.0 + 1.2*peak)}' /proc/meminfo; do sleep 30; done
