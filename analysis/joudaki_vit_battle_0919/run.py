...
RAW = Path.home() / 'Projects/obsidian-research-data/joudaki_vit_battle_0919'
...
def source_hashes():
    paths = sorted((REPO / 'analysis/joudaki_vit_battle_0919').rglob('*.py'))
...
def run(args):
    ...
    cfg = dict(arm=args.arm, seed=args.seed, tasks=args.tasks, steps=args.steps, batch=128,
               lr=1e-4, dtype='float32', engine=args.engine, smoke=args.smoke, model=MODEL_CONFIG)
    ...
    if checkpoint.exists():
        ...
        atomic_json(out / 'provenance.json', dict(config=cfg, git_hash=git_hash, dirty=dirty,
            ...
            dataset_zip_sha256=json.loads((REPO / 'results/joudaki_vit_battle_0919/checks.json').read_text())['dataset_zip_sha256']
                if (REPO / 'results/joudaki_vit_battle_0919/checks.json').exists() else None,
            started=time.time()))
