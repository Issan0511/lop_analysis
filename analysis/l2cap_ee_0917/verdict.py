#!/usr/bin/env python3
"""Verdict for l2cap_ee_0917 (specs/spec_l2cap_ee_0917.md sections 4-5).

    python3 analysis/l2cap_ee_0917/verdict.py [--src results/l2cap_ee_0917/runs]

Reads the 40 shards, applies the registered endpoints and labels, and writes
results/l2cap_ee_0917/{paired.csv, verdict.csv, secondary.csv, timing.csv, summary.md, provenance.json}.
...
Endpoints (spec 4.1), per seed, each minus its own task-1 value:
  ...
  E2 = the SECOND layer's derivative as training uses it (autograd through the float32 ELU, exactly 0
       below z = -16.64): image mean -> arithmetic mean over all 100 units -> arithmetic mean over the
       window's task ends.
...
RUN_ID = "l2cap_ee_0917"
ARMS = ("ref", "cap1", "cap2", "cap12", "cap12_bfix")
CAP_ARMS = ARMS[1:]
MAIN_ARM = "cap12"                          # spec 5.3: the primary comparison is cap12 - ref
CAPPED = {"cap1": ("rows_par", "rows_perp"), "cap2": ("rows_w2",), "cap12": ("rows_par", "rows_perp", "rows_w2"),
...
COMP_LEVEL = 0.95                           # the other three arms, and cap12 for reference
STREAM_KEYS = ("init_sha256", "subset_idx_sha256", "labels_sha256", "batch_sha256",
               "task1_end_state_sha256")
E2_KEY = "dtrain_mean_l2"
UNIT_Q = {"E2": E2_KEY, "g2_exp": "gate_mean_l2", "dtrain0_l2": "dtrain_zero_l2", "dtrain_l1": "dtrain_mean_l1",
          "g1": "gate_mean_l1", "pplus_l2": "pplus_l2", "zbar_l2": "zbar_l2",
...
def analyze(shards: dict) -> dict:
    ...
    res["labels"] = {"main": labels[(MAIN_ARM, MAIN_LEVEL)],
                     "cap12_95": labels[(MAIN_ARM, COMP_LEVEL)],
                     "cap1": labels[("cap1", COMP_LEVEL)],
                     "cap2": labels[("cap2", COMP_LEVEL)],
                     "cap12_bfix": labels[("cap12_bfix", COMP_LEVEL)],
    ...
    res["interaction"] = {ep: paired(diffs("cap12", ep) - diffs("cap1", ep) - diffs("cap2", ep),
                                     COMP_LEVEL) for ep in ("E1", "E2")}
    # registered secondary (spec 4.2-2): the transport itself, the change of the second layer's mean
    # preactivation, arm minus ref, and its two parts zbar2 = q2 ||mu2|| + b2 (q2 along the current mu2)
    res["transport"] = {}
    for arm in CAP_ARMS:
    ...
    for arm in ARMS:
        ...
        for win in wins:                                   # zbar2 = q2 ||mu2|| + b2: the weight part
            lv = []
            for s in valid:
                u = shards[(arm, s)]["units"]
                if "q_l2" in u and "mu2_norm" in u:
                    ends = task_ends(u)
                    lv.append(float(np.mean([np.mean(np.asarray(u["q_l2"][ends[t]], float))
                                             * float(np.asarray(u["mu2_norm"][ends[t]]).ravel()[0])
                                             for t in range(win[0], win[1] + 1)])))
            if lv:
                res["secondary"].append({"arm": arm, "quantity": "level_q2mu2", "window": f"{win[0]}-{win[1]}",
...
def summary_md(res: dict, missing: list, env: dict) -> str:
    ...
    L = [f"# {RUN_ID} — 判定（第2層の行ノルム上限・ELU→ELU・{N_TASKS} タスク）", "",
         "> 自動生成: `analysis/l2cap_ee_0917/verdict.py`。spec: `specs/spec_l2cap_ee_0917.md`。数値は `verdict.csv`・"
         ...
         "", "## 2. 判定", "",
         "| 比較 | 水準 | ラベル | 時間（T_half） |", "|---|---|---|---|",
         f"| **主: cap12 − ref** | 97.5% | **{res['labels']['main']}** | {res['labels']['timing_cap12']} |",
         f"| cap12 − ref（参考） | 95% | {res['labels']['cap12_95']} | |",
         f"| cap1 − ref | 95% | {res['labels']['cap1']} | {res['labels']['timing_cap1']} |",
         f"| cap2 − ref | 95% | {res['labels']['cap2']} | {res['labels']['timing_cap2']} |",
         f"| cap12_bfix − ref | 95% | {res['labels']['cap12_bfix']} | {res['labels']['timing_cap12_bfix']} |", "",
    ...
    L += ["", "交互作用 δ_cap12 − δ_cap1 − δ_cap2（95%）: " + "、".join(
