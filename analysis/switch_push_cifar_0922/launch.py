#!/usr/bin/env python3
"""Job scheduler for switch_push_cifar_0922 (spec §3), forked from le_eps_cifar_0922/launch.py.  Start it detached:

    setsid nohup python3 analysis/switch_push_cifar_0922/launch.py --plan PLAN.json > LOG 2>&1 &

PLAN.json: {"gpu_max": int, "own_max": int, "reserve_gb": float, "min_gpu_mb": int, "retries": int,
            "jobs": [{"arm", "cell", "freeze_steps", "freeze_where", "rss_gb", "threads"}, ...]}
One job = one arm = one process of src/layer_chimera_cifar_0921.py run --cell CELL
--freeze-steps K --freeze-where WHERE --out OUT/ARM
(R = 10: seeds 0-9 x raw).
...
def scan_runs() -> dict[str, int]:
    """arm -> pid of every run of RUNNER in /proc whose --out is OUT/<arm>."""
    found = {}
    for p in os.listdir("/proc"):
        ...
        if o.parent.resolve() == OUT.resolve():
            found[o.name] = int(p)
    return found
    ...
    while True:
        ...
        if not (L / "STOP").exists() and pending and len(running) < plan["own_max"]:
            ...
            if gpu_n < plan["gpu_max"] and avail - j["rss_gb"] >= plan["reserve_gb"]:
                name = j["arm"]
                cmd = [sys.executable, str(RUNNER), "run", "--cell", j["cell"],
                       "--freeze-steps", str(j["freeze_steps"]), "--freeze-where", j["freeze_where"],
                       "--out", str(OUT / name), "--threads", str(j.get("threads", 2))]
