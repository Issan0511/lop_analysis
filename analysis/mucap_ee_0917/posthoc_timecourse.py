#!/usr/bin/env python3
"""mucap_ee_0917 post hoc (NOT registered): per-arm time courses behind the verdict.

    python3 analysis/mucap_ee_0917/posthoc_timecourse.py [--src results/mucap_ee_0917/runs]
        -> results/mucap_ee_0917/posthoc_timecourse.md
...
T = (1, 2, 3, 5, 8, 10, 12, 14, 16, 20, 30, 50, 100)
KEYS = {"online": "online accuracy", "g2": "L2 mean phi'", "mu2": "||mu2||", "c2": "c2 = ||mu2||/sd(e2'a1)",
...
def main() -> None:
    ...
    ap.add_argument("--src", default=str(REPO / "results" / "mucap_ee_0917" / "runs"))
    ap.add_argument("--out", default=str(REPO / "results" / "mucap_ee_0917" / "posthoc_timecourse.md"))
    args = ap.parse_args()
    src = Path(args.src)
    data = {}
    for a in ARMS:
        ...
        for s in range(10):
            ...
            for k in T:
                ...
                m = z[p + "a1mean_l1"][i]
                v = {"online": float(rows[k]["online_acc"]), "g2": z[p + "gate_mean_l2"][i].mean(),
    ...
    L = ["# mucap_ee_0917 — 事後の時間経過（未登録）", "",
         "> `analysis/mucap_ee_0917/posthoc_timecourse.py`。10 seed 平均・タスク終端。always-on = 第1層で 1200 枚すべてが正側のユニット"
