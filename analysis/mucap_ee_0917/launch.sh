#!/usr/bin/env bash
# mucap_ee_0917 -- spec section 8: 4 arms x seeds 0-9 x 100 tasks x 80 epochs (ELU->ELU, ledger off), N at a time, 1 thread each.
...
#   nohup setsid bash analysis/mucap_ee_0917/launch.sh > results/mucap_ee_0917/logs/launch.log 2>&1 &
set -euo pipefail
cd "$(dirname "$0")/../.."
OUT=results/mucap_ee_0917
MOD=src/mucap_ee_run_0917.py
PY=/usr/bin/python3
...
[ -z "$(git status --porcelain -- src analysis/mucap_ee_0917 specs/spec_mucap_ee_0917.md)" ] \
  || { echo "ABORT: uncommitted code or spec"; exit 1; }
git fetch -q origin
[ "$(git rev-list --count '@{u}..HEAD')" = 0 ] || { echo "ABORT: HEAD not pushed"; exit 1; }
N=$($PY - "$MAX_SLOTS" <<'EOF'
import json, sys
c = json.load(open("results/mucap_ee_0917/checks.json"))["S-cost"]
