#!/usr/bin/env bash
# escape_ee_0917 -- spec section 8: 10 jobs (one per seed: the prefix + 12 arms), 1 thread each, at most
# MAX_SLOTS at once.
...
#   setsid nohup bash analysis/escape_ee_0917/launch.sh > results/escape_ee_0917/logs/launch.log 2>&1 &
set -euo pipefail
cd "$(dirname "$0")/../.."
OUT=results/escape_ee_0917
MOD=src/escape_ee_0917.py
PY=/usr/bin/python3
...
[ -z "$(git status --porcelain -- src analysis/escape_ee_0917 specs/spec_escape_ee_0917.md)" ] \
