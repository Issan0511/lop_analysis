"""snake_phase_mnist_0914 launcher with memory watch (spec §8, §11 steps 4-6).  Standard library only.
...
ROOT = HERE.parents[1]
OUT = ROOT / 'results/snake_phase_mnist_0914'
RUNS = OUT / 'runs'
RES = ROOT / 'results/_smoke_snake_phase_mnist_0914/resources/resources.json'
MEMLOG = Path('/tmp/claude-1000/-home-issan-Projects-claude/2ffc1715-3da4-4403-919f-af27962a071a/scratchpad/memlog.txt')
ARMS = ['N06', 'P06', 'V06', 'P06c', 'P06c_k1', 'P06i', 'V06c', 'V06i', 'SNA', 'SNAP', 'SNAV', 'SNAi_P', 'SNAi_V',
        'LIN', 'LR', 'LR_qKp', 'LR_qKpn']
STAGE1 = [(a, s) for s in (0, 1, 2) for a in ('N06', 'LR', 'LIN', 'SNA')]
KB = 1024
...
class Launcher:
    ...
    def _spawn(self, shard):
        ...
        return subprocess.Popen(['nice', '-n', '5', sys.executable, '-m', 'src.snake_phase_mnist_0914', '--arm', arm,
...
def validate_table(rows):
    for arm, seed, out in rows:
        ...
        if arm not in ARMS or not (0 <= seed <= 19):
...
def main_run():
    res = preflight()
    table = [(a, s, RUNS / f'{a}_s{s}') for (a, s) in STAGE1] + \
            [(a, s, RUNS / f'{a}_s{s}') for s in range(20) for a in ARMS if (a, s) not in STAGE1]
    ...
    L.log(f"launcher start rss_peak_kb={res['rss_peak_kb']} window_s={res['wall_seconds']}")
    q1 = [(a, s) for a, s, _ in table[:len(STAGE1)] if not done(a, s)]
    L.run_queue(q1)
    g1 = subprocess.run([sys.executable, str(HERE / 'checks.py'), '--g1-full', str(RUNS)], cwd=ROOT, capture_output=True, text=True)
    L.log(f'g1_full: {g1.stdout.strip()[-300:]}')
    status = json.loads((OUT / 'g1_full.json').read_text())['status']
    if status == 'CODE_MISMATCH':
        L.log('STOP: CODE_MISMATCH at stage 1')
        (OUT / 'launch_done.json').write_text(json.dumps(dict(stopped='CODE_MISMATCH')))
        return
    q2 = [(a, s) for a, s, _ in table[len(STAGE1):] if not done(a, s)]
    L.run_queue(q2)
    ...
    (OUT / 'launch_done.json').write_text(json.dumps(dict(g1=status, final=final, verdict_rc=v.returncode), indent=1))
...
def selftest():
    ...
    try:
        validate_table([('N06', 0, ROOT / 'results/_smoke_snake_phase_mnist_0914/x')]); cases.append(dict(name='refuse_smoke_row', ok=False))
    except SystemExit:
