#!/usr/bin/env python3
"""Job scheduler for rlcifar_mlp_battle_0918 (spec §7).  Start it detached:

    setsid nohup python3 analysis/rlcifar_mlp_battle_0918/launch.py --plan PLAN.json > LOG 2>&1 &

PLAN.json: {"gpu_max": int, "own_max": int, "reserve_gb": float, "min_gpu_mb": int, "retries": int,
            "jobs": [{"arm", "rss_gb", "threads"}, ...]}
One job = one arm = one process of src/rlcifar_mlp_battle_0918.py (R = 20: seeds 0-9 x raw,std).
The GPU is shared with swish_battle_0917's launcher, so the cap counts every CUDA compute process
...
L = OUT / "_launch"
RUNNER = REPO / "src" / "rlcifar_mlp_battle_0918.py"
...
def scan_runs() -> dict[str, int]:
    ...
    for p in os.listdir("/proc"):
        ...
        if len(argv) < 3 or not argv[1].endswith("rlcifar_mlp_battle_0918.py") or argv[2] != "run":
            continue
        if "--out" in argv or "--arm" not in argv:
            continue
        found[argv[argv.index("--arm") + 1]] = int(p)
    return found
